...
启动方式已拆分为两个独立教程：[一键配置与启动](one-click.md) 和 [手动配置与启动](manual.md)。Windows 一键验收直接在项目根目录运行 `start.bat`；脚本会从创建 `.venv` 开始，自动安装依赖、迁移数据库并创建管理员 `admin / Admin@123456`，不需要手动输入虚拟环境命令。手动流程与一键流程分开，请不要同时执行。
