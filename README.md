...
脚本会自动创建 `.venv`、准备 `.env`、按需安装依赖、执行数据库迁移和初始化，并打开两个窗口运行 Django 与 Vue。第一次运行如果还没有管理员账号，脚本会自动进入管理员创建页面。第一次运行前，请确保 MySQL 已按上面的 SQL 创建数据库和专用账号。

如果一键脚本在迁移阶段停止，请先检查根目录 `.env` 中的 MySQL 配置，再重新运行即可。

一键启动完成后直接访问 `http://127.0.0.1:8080/`；后台管理访问 `http://127.0.0.1:8000/admin/`。

### 方式 B：手动运行（用于学习、调试和排错）

手动运行时不要再同时执行 `start.bat`，请在两个终端分别启动后端和前端。
