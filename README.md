...
脚本从虚拟环境创建开始，完全不需要手动创建或激活 `.venv`。它会自动准备 `.env`、按需安装依赖、执行数据库迁移和初始化、创建一键管理员，并打开两个窗口运行 Django 与 Vue。第一次运行前，请确保 MySQL 已按上面的 SQL 创建数据库和专用账号。

如果一键脚本在迁移阶段停止，请先检查根目录 `.env` 中的 MySQL 配置，再重新运行即可。

一键启动完成后直接访问 `http://127.0.0.1:8080/`；后台管理访问 `http://127.0.0.1:8000/admin/`。

一键管理员账号（脚本每次启动都会确保该账号可登录）：
...
### 方式 B：手动运行（用于学习、调试和排错，与方式 A 完全分开）

手动运行时不要执行 `start.bat` 或 `start.ps1`，请完全按照下面步骤在两个终端分别启动后端和前端。手动流程需要你自己创建虚拟环境、安装依赖和创建管理员。
...
只有选择“方式 B：手动运行”时才执行本节；选择一键启动时不需要重复执行。
